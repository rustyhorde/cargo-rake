...
- **`librake`** — all the real logic; the two binaries are thin shells over it.
  - `rakefile.rs` — `Rakefile`/`Target`/`Command` model (a target holds a `Vec<Command>`; each `Command` has `name`/`cmd`/`skip_on_error`), TOML parsing, validation (every target has ≥1 command **or** ≥1 dependency, each `cmd` non-empty), and execution (`Rakefile::run` runs a target after its transitive deps, returning a `RunReport` whose `status` is `Option<ExitStatus>` — `None` when no command ran, i.e. a depends-only target chain — plus the total wall-clock `elapsed`; `run_one` runs the target's commands in order, timing and printing each command's right-justified `Runtime`, and stops at the first non-zero exit unless that command sets `skip_on_error`, returning an `(Option<status>, stop)` pair; `spawn_command` prints a blank line then the command's cargo-style status line (`Running [<name>] -> <program args>`, no `====` banners) and spawns it inheriting stdio). Output is cargo-style: every status line is a right-justified label prefix in one shared column followed by plain info; command/tool prefixes are bold blue, `Runtime`/`Total Runtime` bold green. The column is a compile-time `LABEL_WIDTH` const (currently 13, `"Total Runtime"`) — the longest of the fixed `STATUS_LABELS` (`Running`/runtime labels/tool verbs); command names live in the info after `Running`, so the width never depends on them. `print_label`/`print_justified` write these lines; `format_duration` renders a `Duration` with tiered µs/ms/s/min units and `print_runtime(label, elapsed)` prints a right-justified, bold-green runtime line to stderr (used for per-command `Runtime` and the binaries' `Total Runtime`). Note `std::process::Command` is imported as `ProcessCommand` to avoid clashing with the model `Command`.
  ...
  - `lib.rs` — public re-exports (incl. `Tool`/`SemverCheck`), `DEFAULT_TARGET` const, `exit_code`, `list_targets` (shows `tools:` lines), `format_duration`, `print_runtime`.
